...
(function ($) {
    ...
    var html = {
        ...
        header: function (option) {
            var tag = '<div class="flexDate_header">' +
                ...
                '<b >' + util.getSelectedYear(option.value, option.format) + '</b>-Year' +//jona
                '</span>';
            if (option.types.indexOf(option.type) > 0) {
                tag += '<span class="flexDate_month">' +
                    '<b >' + util.getSelectedMonthStr(option.value, option.format) + '</b>-Month' +
                    '</span>';
            }
        ...
        monthCard: function (option) {
            ...
            for (var i = 1; i <= 12; i++) {
                tag += '<li class="' + (selectedMonth == i ? 'flexDate_selected ' : ' ') + ((!selectedYear || thisYear == selectedYear) && thisMonth == i ? 'flexDate_cur ' : '') +(util.compare(selectedYear+(i < 10 ? '0' + i : i),option.min,option.max)?'':'flexDate_disabled')+ '"><span>' + (i < 10 ? '0' + i : i) + '</span></li>';
            }
            tag += '</ul>';
            return tag;
        },
        ...
        oper: function (option) {
            ...
            return '<div class="flexDate_oper">' +
                (option.clearBtn ? '<a href="javascript:void(0);" >Clear</a> ' : '') +
                (option.todayBtn ? '<a href="javascript:void(0);" >Today</a> ' : '') +
                (option.confirmBtn ? '<a href="javascript:void(0);" >Ok</a>' : '') +
                '</div>';
        ...
        newMonthCard: function (option, targetYear) {
            var tag = '<ul class="flexDate_monthCard">',
                ...
                selectedMonth = util.getSelectedMonth(option.value, option.format);
            for (var i = 1; i <= 12; i++) {
                tag += '<li class="' + (targetYear == selectedYear && selectedMonth == i ? 'flexDate_selected ' : ' ') + ((thisYear == targetYear) && thisMonth == i ? 'flexDate_cur ' : '') +(util.compare(targetYear+(i < 10 ? '0' + i : ''+i),option.min,option.max)?'':'flexDate_disabled')+ '" ><span>' + (i < 10 ? '0' + i : i) + '</span></li>';
            }
            tag += '</ul>';
            return tag;
        },
    ...
    var event = {
        ...
        bindEvent: function (option, page) {
            ...
            page.on('click', '.flexDate_year', function () {//点击年份
            ...
            }).on('click', '.flexDate_oper>a:contains(Empty)', function () {
                assignAndHide('');
            }).on('click', '.flexDate_oper>a:contains(Nowadays)', function () {
                ...
                assignAndHide();
            }).on('click', '.flexDate_oper>a:contains(Submit)', function () {
