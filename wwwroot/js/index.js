...
if (storage) {
    ...
    /**this changes the theme level purple*/

        document.body.className = 'theme-amethyst';
        localStorage.setItem('config-skin', "theme-amethyst");

    /**this changes the theme level 1*/

        //document.body.className = 'theme-greenSea';
        //localStorage.setItem('config-skin', "theme-greenSea");

    /**this changes the theme level 2 recommended*/

        //document.body.className = 'theme-whbl';
        //localStorage.setItem('config-skin', "theme-whbl");

    /**this changes the theme default*/

        //document.body.className = 'theme-blue-gradient';
        //localStorage.setItem('config-skin', "theme-blue-gradient");

    /**this changes the theme red*/

        //document.body.className = 'theme-red';
        //localStorage.setItem('config-skin', "theme-red");
    }
}
else {
    document.body.className = 'theme-amethyst';
...
function GetLoadNav() {
    ...
    $.each(data, function (i) {
        var row = data[i];
        if (row.ParentCodeID == 0) {
            _html += '<li>';
            _html += '<a data-id="' + row.MenuCodeID + '" href="#" class="dropdown-toggle"><i class="' + row.Icon + '"></i><span>' + row.MenuName + '</span><i class="fa fa-angle-right drop-icon"></i></a>';
            ...
            if (childNodes.length > 0) {
                ...
                $.each(childNodes, function (i) {
                    ...
                    _html += '<li>';
                    _html += '<a class="menuItem" data-id="' + subrow.MenuCodeID + '" href="' + subrow.Url + '" data-index="' + subrow.TypeCode + '">' + subrow.MenuName + '</a>';
                    _html += '</li>';
                });
                _html += '</ul>';
            }
